...
def parse_post(post):
    ...
    metadata.setdefault("word_counts", non_stop_words(strip_tags(post_content)))
    return metadata
...
def word_counts_by_year(posts, filter_=None):
    ...
    for year, year_posts in posts_by_year.items():
        ...
        if filter_ == "TOP":
            ...
            items = counter.items()
        words = [{"text": word, "count": count} for word, count in items]
        counts[year] = words
    if filter_ == "TFIDF":
...
if __name__ == "__main__":
    ...
    posts = [parse_post("{}/{}".format(BLOG_DIR, filename)) for filename in os.listdir(BLOG_DIR)]
